...
public class CourseAction extends ActionSupport{
	...
	public String keep(){
		HttpServletRequest request = ServletActionContext.getRequest();
		HttpSession session=request.getSession();
		if(date!=null&&!date.equals(""))//判断字符是否为空
			session.setAttribute("date", date);
		else
		{
			stateCode=0;
	...
	public String add(){
		...
		if(user.getType()==1){//判断是否为学生添加
			Student st=this.getStudentService().findStudentById(user.getId());//这里出现问题
			...
			course.setStudentName("NULL");
			course.setRoomId(roomid);
			course.setRoomName(room.getName());
			course.setStatus(0);
			course.setCourseName(t.getCourseName());
			course.setTeacherId(t.getId());
			course.setTeacherName(t.getName());
			course.setCourseNo(courseNo);
			String[] time=date.split("-");
