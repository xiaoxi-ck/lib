...
<html>
  <head>
	...
	<script type="text/javascript">
		function update(){
			$.post("teacherservice!update.action",{'teacher.name':$("#name").val(),'teacher.phone':$("#phone").val(),'teacher.pwd':$("#pwd").val(),'id':$("#id").val()},function(result){
				if(result.message=="更新成功！"){
					layer.msg('更新成功！', {icon: 4});
				}else{
 ...
 <!-- 头部 -->
    ...
    <div class="container-fluid">
        ...
        <div class="span9">
            <h1 class="page-title">教师信息</h1>
   <s:if test="#session.user.type==2||#session.user.type==3">
  <div class="btn-toolbar">
    <button class="btn btn-primary" onclick="update();"><i class="icon-save"></i> 保存</button>
  <div class="btn-group">
...
<div class="well">
    ...
    <form id="tab">
    	<input type="hidden" id="id" value="<s:property value="#request.teacher.id" />">
        <label>教工号</label>
        <input type="text" class="input-xlarge" name="teacher.no" id="no"  disabled="true" value="<s:property value="#request.teacher.no" />" >
        <label>教工姓名</label>
        <input type="text" class="input-xlarge" name="teacher.name" id="name" disabled="true" value="<s:property value="#request.teacher.name" />" >
        <label>指导课程</label>
        <input type="text" class="input-xlarge" name="teacher.courseName" id="courseName" disabled="true" value="<s:property value="#request.teacher.courseName" />" >
        <label>联系方式</label>
        <input type="text" class="input-xlarge" name="teacher.phone" id="phone" value="<s:property value="#request.teacher.phone" />" >
        <label>密码</label>
        <input type="password" class="input-xlarge" name="teacher.pwd" id="pwd" value="">
    </form>
      </div>
  </div>
