...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		function add(){
			$.post("roomservice!add.action",{'room.name':$("#name").val(),'room.location':$("#location").val(),'room.remark':$("#remark").val(),'room.stuNum':$("#stuNum").val()},function(result){
...
<div class="well">
    ...
    <form id="tab">
        <label>实验室名称</label>
        <input type="text" class="input-xlarge" name="room.name" id="name">
        <label>实验室位置</label>
        <input type="text" class="input-xlarge" name="room.location" id="location">
        <label>容纳人数</label>
        <input type="text" class="input-xlarge" name="room.stuNum" id="stuNum">
        <label>备注</label>
        <input type="text" class="input-xlarge" name="room.remark" id="remark">
    </form>
      </div>
  </div>
...
<div class="modal small hide fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <div class="modal-body">
    ...
    <p class="error-text"><i class="icon-warning-sign modal-icon"></i>点击确定将不保存实验室信息?</p>
  </div>
